...
Этот проект представляет собой не только набор функций для работы с банковскими операциями,
теперь это финансовый анализатор, который обрабатывает данные о финансовых транзакциях из 
различных источников. Изначально проект работал только с JSON-файлами, но теперь он 
расширен для поддержки CSV- и XLSX-форматов. Он позволяет загружать данные о транзакциях 
из различных форматов файлов, фильтровать их по различным параметрам и выполнять анализ.

## Функциональность

Проект включает в себя 13 основных функций:
...
6. `csv_format(file_csv: str)`: Функция для считывания финансовых операций из файла-CSV возвращает список словарей с транзакциями.
7. `excel_format(file_excel: str)`: Функция для считывания финансовых операций из файла-EXCEL возврвщает список словарей с транзакциями.
8. `load_transactions_from_json, load_transactions_from_csv, load_transactions_from_xlsx`: Загрузка транзакций из JSON, CSV и XLSX файлов
9. `filter_transactions_by_status`: Фильтрация транзакций по статусу (EXECUTED, CANCELED, PENDING)
10. `sort_transactions_by_date`: Сортировка транзакций по дате
11. `filter_transactions_by_currency`: Фильтрация транзакций по валюте
12. `search_transactions_by_description`: Поиск транзакций по ключевым словам в описании
13. `count_transactions_by_category`: Подсчет количества транзакций по категориям
...
13. **Тесты функции csv_format:**
    - Тесты для функции считывания финансовых операций из CSV используют Mock и patch

14. **Тесты функции excel_format:**
    - Тесты для функции считывания финансовых операций из Excel используют Mock и patch
    
15. **Тесты функции search_transactions_by_description:**
    - Тест на проверку поиска транзакций по заданной строке в описании

16. **Тесты функции test_filter_transactions_by_status:**
    - Тесты на проверку фильтрации транзакций по их статусу
    
17. **Тесты функции test_sort_transactions_by_date:**
    - Тесты на проверку сортировки транзакций по дате
    
18. **Тесты функции test_filter_transactions_by_currency:**
    - Тесты на проверку фильтрации транзакций по валюте
